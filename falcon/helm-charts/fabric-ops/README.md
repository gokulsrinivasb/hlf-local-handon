...
| `hlf_domain` | The FQDN suffix will be used in CSR generation. Eg `peer0-prg1.test.local.hlf` | `"test.local.hlf"` |
...
| `ca_endpoint` | FQDN of the CA server endpoint with port. `Eg; ica-org1.test.local.hlf:30000` | `""` |
...
| `tlsca_endpoint` | FQDN of the TLSCA server endpoint with port | `"tls-ca.test.local.hlf:30000"` |
| `filestore_endpoint` | FQDN of filestore server endpoint with port | `"http://filestore.test.local.hlf:30001"` |
...
admin_organizations:
  ...
  - org_type: consortium
    ...
    anchor_peers:
    - host: peer0-initialpeerorg.test.local.hlf
      port: "30000"
...
| `tlsca_endpoint` | FQDN of the TLSCA server endpoint with port | `"tls-ca.test.local.hlf:30000"` |
| `orderer_endpoint` | FQDN of the Orderer node endpoint with port | `"orderer0-orderer.test.local.hlf:30000"` |
| `filestore_endpoint` | The filestore endpoint | `"http://filestore.test.local.hlf:30001"` |
| `filestore_ssl` | `true` if `filestore_endpoint` is over https | `false` |
...
anchor_peers:
   - host: peer0-initialpeerorg.test.local.hlf
     port: "30000"
   - host: peer1-initialpeerorg.test.local.hlf
     port: "30000"
   - host: peer2-initialpeerorg.test.local.hlf
     port: "30000"
```
...
| `ica_endpoint` | FQDN of the MSPCA server endpoint with port | `"ica-initialpeerorg.test.local.hlf:30000"` |
| `tlsca_endpoint` | FQDN of the TLSCA server endpoint with port | `"tls-ca.test.local.hlf:30000"` |
| `orderer_endpoint` | FQDN of the Orderer node endpoint with port | `"orderer0-orderer.test.local.hlf:30000"` |
| `hlf_channel` | The channel to update | `""` |
| `admin_identity` | Any valid Admin user identity array in `ica_endpoint`. [Refer](#Admin-identity) | `[]` |
...
organizations:
 - name: org1
   ica_endpoint: ica-org1.test.local.hlf:30000
   anchor_peer: peer0-org1.test.local.hlf
   anchor_peer_port: 30000
   status: active
 - name: org2
   ica_endpoint: ica-org2.test.local.hlf:30000
   anchor_peer: peer0-org2.test.local.hlf
   anchor_peer_port: 30000
   status: disabled
```
...
peer_identities:
 - identity_name: peer0-initialpeerorg
   # peer_endpoint: peer0-initialpeerorg.test.local.hlf:30000 # By default it will use identity_name:peer_internal_service_port
...
additional_orderers:
  - identity_name: orderer3-orderer
    ...
    endpoint: orderer3-orderer.test.local.hlf
    port: 30000
    msp_base_dir: /opt/gopath/src/github.com/hyperledger/fabric/orderer/users
    ica_endpoint: ica-orderer.test.local.hlf:30000
    ica_tls_certfile: /tmp/orderer-ica.cert
...
| `orderer_endpoint` | FQDN of the Orderer node endpoint with port. Make sure that this orderer should not be the one you're trying to update. | `"orderer0-orderer.test.local.hlf:30000"` |
| `filestore_endpoint` | The filestore endpoint | `"http://filestore.test.local.hlf:30001"` |
| `filestore_ssl` | `true` if `filestore_endpoint` is over https | `false` |
| `seconds_to_wait_after_channel_update` | Seconds to pause the script/job activity after one channel update | `15` |
| `tlsca_endpoint` | FQDN of the TLSCA server endpoint with port | `"tls-ca.test.local.hlf:30000"` |
